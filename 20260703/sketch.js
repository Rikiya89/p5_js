...
function drawHUD(loop, psi, coefs) {
  ...
  ctx.fillText('HILBERT SPACE', W / 2, 52 + SAFE_TOP);
  ctx.font      = `normal 19px ${MONO}`;
  ctx.fillStyle = 'rgba(255,255,255,0.36)';
  ctx.fillText('3D PROJECTION SIMULATOR', W / 2, 94 + SAFE_TOP);
  ctx.restore();
  ...
  ctx.fillText('|psi> = SUM c_n |e_n>     c_n = <e_n | psi>', W / 2, 132 + SAFE_TOP);
  ctx.font      = `normal 17px ${MONO}`;
  ctx.fillStyle = 'rgba(255,255,255,0.30)';
  ctx.fillText(`basis=16   loop=${loopStr}   maxAmp=${maxAmp}   dim=INF`, W / 2, 165 + SAFE_TOP);
  ctx.restore();
